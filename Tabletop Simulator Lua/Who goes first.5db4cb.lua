...
function diceRolled()
    local rolledPlayerName = self.getRotationValue()
    for _, player in ipairs(Player.getPlayers()) do
        if (player.steam_name == rolledPlayerName) then
            Global.call("setNewStartPlayer", {playerColor = player.color})
            return
        end
    end
    if Global.getTable("PLAYER")[rolledPlayerName] then
        Global.call("setNewStartPlayer", {playerColor = rolledPlayerName})
    end
end
...
function whoGoesFirst()
    ...
    for k,color in pairs(colors) do
        local colorTable = Global.getTable("ZONE_GUID_PLAYER")
        if colorTable[color] then
            local zone = getObjectFromGUID(colorTable[color])
